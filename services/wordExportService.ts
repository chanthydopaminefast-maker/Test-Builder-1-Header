...
function buildWordSafeHeader(
  ...
  const accentColor = withColor
    ? accentColors[paperDesign] || activeRulerColor || "#ea580c"
    : "#000000";
  ...
  if (withColor && !isTopBottomLineEnabled) {
    if (globalLayout === 1) {
      ...
      topBarColor = "#9333ea";
      leftBarColor = "#9333ea";
    } else if (globalLayout === 6) {
      // Sky
      topBarColor = "#0284c7";
      leftBarColor = "#0ea5e9";
    } else if (globalLayout === 15) {
      // Deep Ocean
      topBarColor = "#1e3a8a";
      leftBarColor = "#3b82f6";
    }
  ...
  const headerTopBarColor =
    paperDesign === 13 && withColor && !isTopBottomLineEnabled
      ? "#b91c1c"
      : topBarColor;
...
export const exportToWord = (
  ...
  while (
    ...
    (tempDiv.firstElementChild?.tagName === "DIV" ||
      tempDiv.firstElementChild?.classList.contains("prose"))
  ) {
  ...
  if (baseLayout === 3 || activeDesign === "3" || activeDesign === 3) {
    ...
    if (
      !topBottomLineColor ||
      topBottomLineColor === "white" ||
      topBottomLineColor === "transparent"
    ) {
      const vibrantColors = [
        "#f97316",
        "#3b82f6",
  ...
  } else if (globalLayout === 1 || activeDesign === "design-playful")
    activeRulerColor = "#059669";
  else if (globalLayout === 2 || activeDesign === "design-eco")
    activeRulerColor = "#059669";
  ...
  if (brandSettings) {
    ...
    tempDiv
      .querySelectorAll(
        ".school-header, .worksheet-header, .academic-evaluation, h1",
      )
    ...
    const schoolNamePart = (brandSettings.schoolName || "")
      .split(" ")[0]
      .toUpperCase();
  ...
  tempDiv
    .querySelectorAll('.options-table td, [data-type="mcq-options"] td')
    .forEach((td) => {
  ...
  const images = [
    ...Array.from(tempDiv.querySelectorAll("img")),
    ...Array.from(headerDiv.querySelectorAll("img")),
  ];
  for (const img of images) {
    const originalWidth = img.width || 550;
    const isLogo =
      img.style.maxHeight === "80pt" ||
      img.classList.contains("logo") ||
      headerDiv.contains(img);
  ...
  if (!isInstructionBackgroundEnabled) {
    const allHeaders = tempDiv.querySelectorAll(
      ".header-row, .part-header, .instruction-header, h2, h3",
    );
  ...
  const shapeBoxes = tempDiv.querySelectorAll(
    ".shape-drawn-1, .shape-drawn-2, .shape-drawn-3, .shape-box",
  );
  ...
  mcqElements.forEach((el) => {
    ...
    const isAnswerKey =
      el.closest(".answer-key-section") || el.closest(".answer-key");
    ...
    if (mcqStyle > 0) {
      ...
      if (["A", "B", "C", "D", "a", "b", "c", "d"].includes(text)) {
        ...
        if (isColorExportEnabled) {
          if (
            activeDesign === "design-eco" &&
            (mcqStyle === 1 || mcqStyle === 15)
          ) {
            borderColor = "#059669";
            bgColor = "#ecfdf5";
            textColor = "#065f46";
            ...
            (activeDesign === "design-modern-blue" ||
              activeDesign === "design-bold-red") &&
            (mcqStyle === 1 || mcqStyle === 15)
          ) {
            borderColor = "#2563eb";
            bgColor = "#eff6ff";
        ...
        if (mcqStyle === 1 || mcqStyle === 15) {
          ...
          // NEW: Small Round Style (5pt font, 0.5pt stroke)
          ...
          let borderType =
            mcqStyle === 11
              ? 'dashstyle="solid" strokeweight="1pt"'
              : 'dashstyle="dot" strokeweight="1.2pt"';
          let htmlBorder =
            mcqStyle === 11 ? "1pt solid black" : "1.2pt dotted black";
  ...
  // Instruction Rulers
  const existingRulers = tempDiv.querySelectorAll(
    '[class*="instruction-ruler-"]',
  );
  existingRulers.forEach((ruler) => {
    const el = ruler as HTMLElement;
    const styleNum = parseInt(
      el.className.match(/instruction-ruler-(\d+)/)?.[1] || "0",
    );
    el.style.width = "100%";
    ...
    el.innerHTML = "";
    if (styleNum === 1)
      el.style.borderBottom = `1.5pt solid ${activeRulerColor}`;
    else if (styleNum === 2)
      el.style.borderBottom = `2pt dashed ${activeRulerColor}`;
    else if (styleNum === 3)
      el.style.borderBottom = `4pt double ${activeRulerColor}`;
    else if (styleNum === 4)
      el.style.borderBottom = `4pt solid ${activeRulerColor}`;
    else if (styleNum === 5 || styleNum === 6)
      el.style.borderBottom = `1pt solid ${activeRulerColor}`;

    if (styleNum > 0)
      el.style.setProperty("mso-border-bottom-alt", el.style.borderBottom);
  });

  // 1. TRANSFORM MCQ OPTIONS INTO WORD-SAFE LAYOUT TABLES
  const optionsTables = tempDiv.querySelectorAll(
    '.options-table, [data-type="mcq-options"]',
  );
  optionsTables.forEach((table) => {
    ...
    layoutTable.classList.add("mcq-layout-table");
  ...
  const answerKeySections = tempDiv.querySelectorAll(
  ...
  tables.forEach((table) => {
    if (table.classList.contains("mcq-layout-table")) return;
    ...
    if (isNested) {
      ...
      table.querySelectorAll("td, th").forEach((cell) => {
        ...
        const isMcqRow = /^[A-D]\./i.test(cell.textContent?.trim() || "") || /^[1-9][0-9]*\./.test(cell.textContent?.trim() || "");
        ...
        const isHeader =
          ...
          cell.tagName === "TH" ||
          (row?.rowIndex === 0 && !isMcqRow)
          );

        // GRID TABLE DESIGNS (10-17) MAPPING
        const gridStyleMap: Record<
          number,
        ...
        const defaultBorderColor = activeGrid
          ? activeGrid.borderColor
          : "#cbd5e1";

        c.style.border = `0.5pt solid ${defaultBorderColor}`;
        c.style.setProperty(
          "mso-border-alt",
          `0.5pt solid ${defaultBorderColor}`,
        );
        ...
        if (isHeader) {
          ...
          if (isInstructionBackgroundEnabled) {
            ...
            const style = headerStyles[instructionHeaderStyle] || {
              bg: "#f0fdf4",
              color: "#166534",
            };
            bg = style.bg;
            ...
            if (style.textTransform)
              c.style.textTransform = style.textTransform;
          ...
          // EXPLICITLY force white background on non-header cells to prevent color spill
          c.style.backgroundColor = "#ffffff";
          c.style.setProperty("mso-shading", "#ffffff");
      ...
      if (
        table.classList.contains("zebra") ||
        table.getAttribute("data-type") === "zebra"
      ) {
        rows.forEach((row, idx) => {
          if (idx % 2 === 1) {
            // odd index = even row (1, 3, 5...)
      ...
      if (isRulerLayout) {
        const isActuallyTwoCol = Array.from(table.rows).some(
          (r) => r.cells.length === 2,
        );
        if (isActuallyTwoCol) {
          table.style.border = "none";
          ...
          Array.from(table.rows).forEach((row) => {
            ...
            if (isHeaderRow) {
              ...
              row.cells[0].style.setProperty(
                "mso-border-bottom-alt",
                `solid ${activeRulerColor} 1.5pt`,
              );
              return;
            }
            ...
            if (row.cells.length === 2) {
              ...
              leftCell.style.setProperty(
                "mso-border-right-alt",
                `solid ${activeRulerColor} 2.5pt`,
              );
  ...
  if (
    ...
    !firstChild.querySelector("img, table")
  ) {
    firstChild.remove();
  }
  ...
  if (
    sections.length === 1 &&
    (sections[0].classList.contains("prose") || sections[0].tagName === "DIV")
  ) {
  ...
  sections.forEach((el) => {
    ...
    // We target headings, part-headers, stylized instruction/relaxation boxes, or any pure-text element that exactly duplicates a header.
    const isPotentiallyRedundant = 
      ...
      htmlEl.classList.contains("instruction-header") ||
      ...
      htmlEl.classList.contains("header-row") ||
      htmlEl.tagName === "STRONG" || 
      htmlEl.tagName === "B" ||
      Array.from(htmlEl.classList).some(cls => cls.startsWith("bg-relax-")) ||
      // If any block-level element matches a known instruction and is pure text, treat it as redundant
      (seenTexts.has(normalizedText) && normalizedText.length > 5 && !htmlEl.querySelector("table") && !htmlEl.querySelector("img") && !htmlEl.querySelector("ol") && !htmlEl.querySelector("ul"));
    ...
    if (
      htmlEl.classList.contains("shape-drawn-1") ||
      htmlEl.classList.contains("shape-drawn-2") ||
      htmlEl.classList.contains("shape-drawn-3")
    ) {
    ...
    if (htmlEl.tagName === "TABLE") {
      ...
      if (firstRow) {
        ...
        const rowNormalized = rowText.toLowerCase().replace(/[^a-z0-9\s]/g, '');
        if (rowNormalized && seenTexts.has(rowNormalized)) {
  ...
  const frameStyle = "";
  const pageBorderStyle = isFrameEnabled
    ? `border: 3.0pt solid ${activeRulerColor}; padding: 24pt 24pt 24pt 24pt; mso-page-border-z-order: front; mso-page-border-surround-header:no; mso-page-border-surround-footer:no;`
    : "";
  const physicalFrameSyle = isFrameEnabled
    ? `border: 3.0pt solid ${activeRulerColor}; mso-border-alt: 3.5pt solid ${activeRulerColor}; padding: 10pt;`
    : "";
  ...
  if (globalLayout === 1 && isTopBottomLineEnabled) {
    // Orange Mix + Top-bottom line enabled
    ...
    // Orange Mix, default (no extra borders if top-bottom line is off)
    ...
    if (isTopBottomLineEnabled)
      paperTdStyle += `border-left: 12pt solid ${topBottomLineColor}; mso-border-left-alt: 12pt solid ${topBottomLineColor};`;
    bodyBgColor = "#f0fdf4";
    ...
    if (globalLayout === 10) {
      ...
      color1 = "#fde047";
      color2 = "#fcd34d";
    } else if (globalLayout === 11) {
      // Flowers
      icon = "✿";
      color1 = "#f9a8d4";
      color2 = "#fbcfe8";
    } else if (globalLayout === 12) {
      // Hearts
      icon = "♥";
      color1 = "#fca5a5";
      color2 = "#fecaca";
    } else if (globalLayout === 13) {
      // Bubbles
      icon = "〇";
      color1 = "#7dd3fc";
      color2 = "#bae6fd";
    } else if (globalLayout === 14) {
      // Leaves
      icon = "🌿";
      color1 = "#bef264";
      color2 = "#d9f99d";
  ...
  if (isStarLineEnabled) {
    ...
    if (!headerWatermarks) {
      ...
      headerWatermarks = headerWatermarks.replace(
        "<![endif]-->",
        `${starVml}<![endif]-->`,
      );
    }
